...
let canvasWidth = 300,
  canvasHeight = 350,
  marginLeft = canvasWidth * 0.17,
  marginRight = canvasWidth * 0.9,
  marginTop = canvasHeight * 0.3,
  marginBottom = canvasHeight * 0.9,
  ...
  doubleDigitNoteTextSize = singleDigitNoteTextSize * 0.6,
  fontForFrets = 'Arial',
  title = '',
...
for (i = 0; i <= numberHorzontalLines - 1; i++) {
  if (i === 0) {
    ...
    markerY[i] =
      marginTop +
      distanceBetweenHorizontalLines / 2 +
      distanceBetweenHorizontalLines * (i - 1)
  }
}
...
function drawOpenShapesAboveNut(x, y, state) {
  if (state === '') {
    ...
    ctx.fillText(
      'o',
      x,
      aboveTopNutYRectPosition + openWhiteRectangleHeight / 2
    )
    ctx.closePath()
    return 'circle'
