...
let canvasWidth = 1100,
  ...
  whiteRectangleWidth = distanceBetweenVerticalLines * 0.7,
  whiteRectangleHeight = distanceBetweenHorizontalLines * 0.95,
  openWhiteRectangleWidth = distanceBetweenVerticalLines * 0.5,
  openWhiteRectangleHeight = distanceBetweenHorizontalLines * 0.7,
  ...
  fontSizeForTitle = aboveTopNutYRectPosition * 0.6, //in pixels
  fontSizeForSubtitle = aboveTopNutYRectPosition * 0.3, //in pixel
  fontForTitle = 'Arial',
  leftSideTextMarker = marginLeft / 3, //adjust as needed for text on left side
  singleDigitNoteTextSize =
    distanceBetweenVerticalLines * circleSizePercentage * 1.5,
  ...
  fontForBrand = 'sans-serif',
  fontSizeForBrand = 24,
  fretMarkerDotSize = distanceBetweenVerticalLines * 0.2 //end settings
...
function Note(x, y, state) {
  ...
  this.y = y
  this.state = ''
  this.xClickableLeft = this.x - distanceBetweenVerticalLines * 0.3
  this.xClickableRight = this.x + distanceBetweenVerticalLines * 0.3
  this.yClickableBottom = this.y - distanceBetweenVerticalLines * 0.3
  this.yClickableTop = this.y + distanceBetweenVerticalLines * 0.3
}
...
for (i = 0; i <= numberVerticalLines; i++) {
  if (i === 0) {
    markerX[i] = marginLeft * 0.5
  } else if (i === 1) {
    markerX[i] = marginLeft + distanceBetweenVerticalLines / 2
  } else {
    markerX[i] =
      marginLeft +
      distanceBetweenVerticalLines / 2 +
      distanceBetweenVerticalLines * (i - 1)
  }
}
...
function setSaveTitle() {}

// Slugify a string for saving image
...
//draw dots on 3,5,7,etc...
ctx.beginPath()
ctx.fillStyle = 'lightgrey'
ctx.arc(
  markerX[3],
  marginTop + guitarHeight / 2,
  fretMarkerDotSize,
  0,
  2 * Math.PI
)
...
//draw dots for 12th fret
ctx.beginPath()
ctx.fillStyle = 'lightgrey'
ctx.arc(
  markerX[12],
  marginTop + guitarHeight * 0.28,
  fretMarkerDotSize,
  0,
  2 * Math.PI
)
...
$('#chordSubtitleButton').click(function () {
  title = $('#chordSubtitle').val()
  printSubtitle(title)
})

$('#chordSubtitle').keypress(function (e) {
  if (e.which == 13) {
    subtitle = $('#chordSubtitle').val()
...
$('#chordClearSubtitleButton').click(function () {
  clearSubtitle()
})
...
function printTitle(t) {
  ctx.beginPath()
  ctx.fillStyle = 'white'
  ctx.rect(1, 1, canvasWidth - 2, aboveTopNutYRectPosition - 15)
  ctx.fill()
  ctx.closePath()
...
//subtitle functions
function printSubtitle(t) {
  ctx.beginPath()
  ctx.fillStyle = 'white'
  ctx.rect(
    1,
    aboveTopNutYRectPosition - 15,
    canvasWidth - 2,
    marginTop - aboveTopNutYRectPosition - 1
  )
  ctx.fill()
  ctx.closePath()
  ...
  ctx.textAlign = 'left'
  // ctx.textBaseline = 'left'
  ctx.fillText(t, canvasWidth * 0.05, aboveTopNutYRectPosition - 5, canvasWidth)
  ctx.closePath()

  $('#chordSubtitle').val('').blur()
}
...
function drawShapes(x, y, state) {
  if (state === '') {
    ...
    ctx.fillStyle = 'white'
    ctx.rect(
      x - whiteRectangleWidth / 2,
      y - whiteRectangleHeight / 2,
      whiteRectangleWidth,
    ...
    ctx.arc(
      x,
      y,
      distanceBetweenVerticalLines * circleSizePercentage,
      0,
      2 * Math.PI
    )
    ctx.fill()
  ...
  if (state === 'circle') {
    ...
    ctx.fillStyle = 'black'
    ctx.rect(
      x - whiteRectangleWidth / 2 + 1,
      y - whiteRectangleHeight / 2 + 1,
      whiteRectangleWidth - 2,
      whiteRectangleHeight - 1
    )
...
function drawOpenShapesAboveNut(x, y, state) {
  if (state === '') {
    ...
    ctx.fillStyle = 'white'
    ctx.rect(
      x - openWhiteRectangleWidth / 2,
      y - openWhiteRectangleHeight / 2,
      openWhiteRectangleWidth,
      openWhiteRectangleHeight
    )
    ctx.fill()
    ctx.closePath()
    ...
    ctx.fillStyle = 'white'
    ctx.rect(
      x - openWhiteRectangleWidth / 2 - 1,
      y - openWhiteRectangleHeight / 2 - 1,
      openWhiteRectangleWidth + 2,
      openWhiteRectangleHeight + 2
    )
    ctx.fill()
    ctx.closePath()
